...
//		inventoryStockIssue.inventoryBranchCode().sendKeys(requestReferenceNo[1]);
		inventoryStockIssue.inventoryBranchCode().sendKeys(inventoryStockIssueTestData.inventoryBranchCode);
...
//    	waitHelper.waitForElement(driver, 3000, inventoryStockIssue.inventoryStockIssueSubmitButton());
    ...
    public void verify_the_inventory_stock_issue_record_is_appeared_in_the_temp_view() throws IOException, ParseException, InterruptedException  {
...
//    	waitHelper.waitForElementVisible(inventoryStockIssue.inventoryStockIssueReferenceNumber(), 2000, 200);
    	waitHelper.waitForElementwithFluentwait(driver, inventoryStockIssue.inventoryStockIssueReferenceNumber());
    	String str = jsonData.readInventoryManagementReqRefNumber();
    	System.out.println(str);
    	Assert.assertEquals(jsonData.readInventoryManagementReqRefNumber(), inventoryStockIssue.inventoryStockIssueReferenceNumber().getText());
...
//    	waitHelper.waitForElementwithFluentwait(driver, inventoryStockIssue.inventoryStockIssueUserName());
    	waitHelper.waitForElementToVisibleWithFluentWait(driver, inventoryStockIssue.inventoryStockIssueUserName(), 3, 2);
...
//    	waitHelper.waitForElement(driver, 3000, inventoryStockIssue.inventoryStockIssueLogoutButton());
    	Thread.sleep(1500);
//    	waitHelper.waitForElementToVisibleWithFluentWait(driver, inventoryStockIssue.inventoryStockIssueLogoutButton(), 3, 2);
    	waitHelper.waitForElementwithFluentwait(driver, inventoryStockIssue.inventoryStockIssueLogoutButton());
...
//				driver.findElement(By.xpath(before_xpath + jsonData.readReferancedata() + after_xpath)));
		waitHelper.waitForElementwithFluentwait(driver, driver.findElement(By.xpath(before_xpath + jsonData.readReferancedata() + after_xpath)));
...
//    	reviewerObj.reviewerUserName().click();
    	clicksAndActionsHelper.JSEClick(reviewerObj.reviewerUserName());
...
//    	waitHelper.waitForElement(driver, 3000, kubsChecker.checkerAlertClose());
    ...
    public void approve_the_record() throws Throwable {
		...
		try
		{
			//span[contains(text(),'')]/ancestor::datatable-body-cell/preceding-sibling::datatable-body-cell/div/ion-buttons/ion-button
//			(//span[contains(text(),'STOCK_ISSUANCE')]/ancestor::div[2]/child::datatable-body-cell[2]/div/span)[2]
			String before_xpath = "//span[contains(text(),'";
