...
//		waitHelper.waitForElement(driver, 5000, arapObj.ARAP_GetcancelBpName());
	...
	public void navigate_the_azentio_url() throws Throwable {
		...
		String kubsApplicationUrl= configFileReader.getApplicationUrl();
		//kubsLogin.loginToAzentioAppByMaker();
		ExcelData excelData = new ExcelData("C:\\Users\\inindc00073\\Downloads\\KUBSTestData.xlsx","KUBS_LoginCredentials","Stage");
        Map<String, String> testData = excelData.getTestdata("Maker1");
		//driver.get(configFileReader.getApplicationUrl());
        driver.get(kubsApplicationUrl);
        //kubsLogin.loginToAzentioAppByMaker(testdata.get("UserID"),testdata.get("Password"));
        //kubsLogin.loginToAzentioAppByMaker(testData.get("UserID"),testData.get("Password"));
		Thread.sleep(1000);
	}
	...
	public void store_the_referance_id_and_open_the_record() throws Throwable {
		...
		excelData.updateTestData("AT-CM-T001_D1", "Reference ID", Referance_id);
		for (int i = 0; i < 20; i++) {
			try {
	...
	public void enter_payment_option() throws Throwable {
		...
		Thread.sleep(500);
		arapObj.accountsPayablePayementSettlementPaymentOption().sendKeys(Keys.DOWN);
...
//		waitHelper.waitForElement(driver, 2000, arapObj.accountsReceivable_Advance_GetBp());
	...
	public void give_stored_business_partner_name() throws Throwable {
		...
		arapObj.accountsPayablePayementSettlementBpName().sendKeys(BPNumber);
		Thread.sleep(500);
		arapObj.accountsPayablePayementSettlementBpName().sendKeys(Keys.DOWN);
		arapObj.accountsPayablePayementSettlementBpName().sendKeys(Keys.ENTER);
...
//	}
	...
	public void give_remark_and_submit() throws Throwable {
		...
		Thread.sleep(1000);
		clickAndActionHelper.moveToElement(reviewerObj.reviewerAlertSubmitButton());
	...
	public void navigate_as_a_checker() throws Throwable {
		...
		//kubsLogin.loginToAzentioAppAsChecker(testData.get("Checker ID"));
	...
	public void click_the_first_action_icon() throws Throwable {
		...
		for (int i = 0; i < 50; i++) {
			try {
				javaScriptHelper = new JavascriptHelper();
				javaScriptHelper.JavaScriptHelper(driver);
				javaScriptHelper.JSEClick(driver.findElement(By.xpath("//span[contains(text(),'"
