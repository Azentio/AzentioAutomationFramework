...
public class BUDGET_BudgetDefinitionChecker extends BaseClass {
   ...
   // this step we enter the alert remark for thei step we can enter the possitive remark because This record going to approve
	...
	public void write_negative_remark_and_submit_it() {
	 ...
	 @Then("^validate the budget record from checker$")
	    public void validate_the_budget_record_from_checker() throws Throwable {
		 Thread.sleep(1000);
			javascriptHelper.JavaScriptHelper(driver);
	        String budgetCode = (String) javascriptHelper
					.executeScript("return document.getElementsByClassName('native-input sc-ion-input-md')[1].value");
			...
			String budgetRemark = (String) javascriptHelper
					.executeScript("return document.getElementsByClassName('native-input sc-ion-input-md')[4].value");
			Assert.assertEquals(budgetCode, budgetDefinitionTestData.budgetCode);
			Assert.assertEquals(budgetType, budgetDefinitionTestData.budgetType);
			if(budgetType.equalsIgnoreCase("YEARLY"))
			{
			Assert.assertEquals(budgetname, budgetDefinitionTestData.uatYearlyBudgetType);
			}
			else if(budgetType.equalsIgnoreCase("HALFYEARLY"))
			{
				Assert.assertEquals(budgetname, budgetDefinitionTestData.uatHalfYearlyBudgetType);

			}
			else if(budgetType.equalsIgnoreCase("MONTHLY"))
			{
				Assert.assertEquals(budgetname, budgetDefinitionTestData.uatMonthlyBudgetType);

			}
			else if(budgetType.equalsIgnoreCase("QUARTERLY"))
			{
				Assert.assertEquals(budgetname, budgetDefinitionTestData.uatQuarterlyBudgetType);
			}
