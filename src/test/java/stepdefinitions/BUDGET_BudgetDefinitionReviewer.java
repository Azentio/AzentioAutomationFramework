...
public class BUDGET_BudgetDefinitionReviewer extends BaseClass {
	...
	BUDGET_BudgetCreationTestDataType budgetData = jsonconfig.getBudgetdataByName("Maker");
	...
	public void navigate_to_URL_and_login_as_a_reviewer() throws IOException, ParseException {
		...
		kubsLogin.logintoAzentioappReviewer(user, json.readdata());
	...
	public void click_on_notification_icon_to_check_reviewer_can_get_a_record_whic_is_submited_by_maker()
		...
		waitHelper.waitForElement(driver, 3000,
				driver.findElement(By.xpath(before_xpath + json.readReferancedata() + after_xpath)));
		Assert.assertTrue(
				driver.findElement(By.xpath(before_xpath + json.readReferancedata() + after_xpath)).isDisplayed());
		String referenceID = driver.findElement(By.xpath(before_xpath + json.readReferancedata() + after_xpath))
				.getText();
		System.out.println("Reference ID is" + referenceID);
		System.out.println("Json reference is" + json.readReferancedata());
		Assert.assertEquals(referenceID, json.readReferancedata());
...
// after validation we have to click on that perticular record
		...
		driver.findElement(By.xpath(before_xpath + json.readReferancedata() + after_xpath)).click();
	...
	public void verify_the_budget_record_is_matched_with_maker_submited_record() throws Throwable {
		...
		String budgetTpe = reviewerObj.budgetType().getText();
		...
		Assert.assertEquals(budgetCode, budgetData.BudgetCode);
		Assert.assertEquals(budgetTpe, budgetData.BudgetType);
		Assert.assertEquals(budgetname, budgetData.BudgetName);
		Assert.assertEquals(warningPercentage, budgetData.WarningPecentage);
		Assert.assertEquals(budgetRemark, budgetData.Remark);
	}
...
// when click on submit button alert will pop up and we have to enter possitive remark in alert
		waitHelper.waitForElement(driver, 2000, reviewerObj.reviewerAlertRemarks());
		reviewerObj.reviewerAlertRemarks().sendKeys(commentsfromapprover.PossitiveRemarks);
		...
		Assert.assertTrue(reviewerObj.reviewerApprovalStatus().isDisplayed());
		String reviewerApprovalStatus = reviewerObj.reviewerApprovalStatus().getText();
	...
	public void enter_remark_then_submit_in_reviewer_stage() {
		...
		reviewerObj.reviewerAlertRemarks().sendKeys(commentsfromapprover.NegativeRemarks);
		reviewerObj.reviewerAlertSubmitButton().click();
		waitHelper.waitForElement(driver, 3000, reviewerObj.reviewerApprovalStatus());
		Assert.assertTrue(reviewerObj.reviewerApprovalStatus().isDisplayed());
		...
		String approvalStatus = reviewerObj.reviewerApprovalStatus().getText();
	...
	public void verify_the_updated_budget_record_is_matched_with_maker_submited_record() throws Throwable {
		...
		Assert.assertEquals(budgetTpe, budgetData.UpdatedBudgetType);
